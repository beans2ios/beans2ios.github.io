...
- 기능의 설명:  
기존 ObjectiveC로 구현된 레거시 코드가 있던 상세검색 기능을 리뉴얼 요청에 따라 새롭게 구현해주었습니다. 
![sample](/assets/image/sample.png)  

- 구현 방식의 이점:
기존의 코드의 Swift 방식의 Swift로 구현하여 신속성,안정성, 추후 유지보수의 편의성을 증가시켰습니다.
구현시 추후 유지보수가 편하도록 종속적이지 않게 구현하려고 했습니다.
...
자동화 배포 적용
-------------
- 적용된 기술: fastlane, slack webhook  

- 기능의 설명:  
![sample](/assets/image/sample.png)  
적용 목표는 gitlab 서버와 빌드 머신을 사용하여 git push가 되면 appStore의 배포가 자동으로 되게 하려고 했습니다.  

현실적으로 사내에 적용가능한 빌드 머신이 없으며, gitlab의 서버 이용비용에 대한 지원이 없어 적용이 힘들게 되어,
로컬에서라도 자동화 배포를 적용해보자 해서 git에서의 push가 되면 testFlight에 업로드 되도록 구현하였습니다.  

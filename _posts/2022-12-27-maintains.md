...
개발 언어 전환한다는 것에 의미를 두지 않고 처음부터 깔끔하게 코드를 짜려면 어떻게 하고 추후 유지보수를 용이하게 하려면 어떻게 할것인가?를 생각하며 작업하다보니 코드의 설계나 디자인 패턴 적용이나, 기능을 수행하는 Class의 분리 등 학습한 내용을 코드에 적용하는 경험이 되었습니다.  
또,Swift언어를 사용하므로서 간단하게 구현되는 코드를 보며 뿌듯함을 느꼈습니다.  
예를 들어 기존 레거시 코드에서는 Networking 통신을 하는경우 일일히 Dictionary로 파싱하는 절차가 굉장히 코드도 지저분하고 힘들었었습니다.  
